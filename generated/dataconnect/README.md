...
You can execute the `GetCreatorTrialUsage` query using the following action shortcut function, or by calling `executeQuery()` after calling the following `QueryRef` function, both of which are defined in [dataconnect/index.d.ts](./index.d.ts):
```typescript
getCreatorTrialUsage(vars: GetCreatorTrialUsageVariables, options?: ExecuteQueryOptions): QueryPromise<GetCreatorTrialUsageData, GetCreatorTrialUsageVariables>;
...
You can also pass in a `DataConnect` instance to the action shortcut function or `QueryRef` function.
```typescript
getCreatorTrialUsage(dc: DataConnect, vars: GetCreatorTrialUsageVariables, options?: ExecuteQueryOptions): QueryPromise<GetCreatorTrialUsageData, GetCreatorTrialUsageVariables>;
...
If you need the name of the operation without creating a ref, you can retrieve the operation name by calling the `operationName` property on the getCreatorTrialUsageRef:
```typescript
const name = getCreatorTrialUsageRef.operationName;
console.log(name);
```
...
```
### Return Type
Recall that executing the `GetCreatorTrialUsage` query returns a `QueryPromise` that resolves to an object with a `data` property.

The `data` property is an object of type `GetCreatorTrialUsageData`, which is defined in [dataconnect/index.d.ts](./index.d.ts). It has the following fields:
```typescript
...
```
### Using `GetCreatorTrialUsage`'s action shortcut function

```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, getCreatorTrialUsage, GetCreatorTrialUsageVariables } from '@bondcircle/dataconnect';

// The `GetCreatorTrialUsage` query requires an argument of type `GetCreatorTrialUsageVariables`:
const getCreatorTrialUsageVars: GetCreatorTrialUsageVariables = {
  creatorId: ..., 
};

// Call the `getCreatorTrialUsage()` function to execute the query.
// You can use the `await` keyword to wait for the promise to resolve.
const { data } = await getCreatorTrialUsage(getCreatorTrialUsageVars);
// Variables can be defined inline as well.
const { data } = await getCreatorTrialUsage({ creatorId: ..., });

// You can also pass in a `DataConnect` instance to the action shortcut function.
const dataConnect = getDataConnect(connectorConfig);
const { data } = await getCreatorTrialUsage(dataConnect, getCreatorTrialUsageVars);

console.log(data.creatorTrialUsage);

// Or, you can use the `Promise` API.
getCreatorTrialUsage(getCreatorTrialUsageVars).then((response) => {
  const data = response.data;
  console.log(data.creatorTrialUsage);
});
```

### Using `GetCreatorTrialUsage`'s `QueryRef` function

```typescript
import { getDataConnect, executeQuery } from 'firebase/data-connect';
import { connectorConfig, getCreatorTrialUsageRef, GetCreatorTrialUsageVariables } from '@bondcircle/dataconnect';
...
// Call the `getCreatorTrialUsageRef()` function to get a reference to the query.
const ref = getCreatorTrialUsageRef(getCreatorTrialUsageVars);
// Variables can be defined inline as well.
const ref = getCreatorTrialUsageRef({ creatorId: ..., });

// You can also pass in a `DataConnect` instance to the `QueryRef` function.
const dataConnect = getDataConnect(connectorConfig);
const ref = getCreatorTrialUsageRef(dataConnect, getCreatorTrialUsageVars);
...
You can execute the `GetCirclePricingState` query using the following action shortcut function, or by calling `executeQuery()` after calling the following `QueryRef` function, both of which are defined in [dataconnect/index.d.ts](./index.d.ts):
```typescript
getCirclePricingState(vars: GetCirclePricingStateVariables, options?: ExecuteQueryOptions): QueryPromise<GetCirclePricingStateData, GetCirclePricingStateVariables>;
...
You can also pass in a `DataConnect` instance to the action shortcut function or `QueryRef` function.
```typescript
getCirclePricingState(dc: DataConnect, vars: GetCirclePricingStateVariables, options?: ExecuteQueryOptions): QueryPromise<GetCirclePricingStateData, GetCirclePricingStateVariables>;
...
If you need the name of the operation without creating a ref, you can retrieve the operation name by calling the `operationName` property on the getCirclePricingStateRef:
```typescript
const name = getCirclePricingStateRef.operationName;
console.log(name);
```
...
```
### Return Type
Recall that executing the `GetCirclePricingState` query returns a `QueryPromise` that resolves to an object with a `data` property.

The `data` property is an object of type `GetCirclePricingStateData`, which is defined in [dataconnect/index.d.ts](./index.d.ts). It has the following fields:
```typescript
...
```
### Using `GetCirclePricingState`'s action shortcut function

```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, getCirclePricingState, GetCirclePricingStateVariables } from '@bondcircle/dataconnect';

// The `GetCirclePricingState` query requires an argument of type `GetCirclePricingStateVariables`:
const getCirclePricingStateVars: GetCirclePricingStateVariables = {
  circleId: ..., 
};

// Call the `getCirclePricingState()` function to execute the query.
// You can use the `await` keyword to wait for the promise to resolve.
const { data } = await getCirclePricingState(getCirclePricingStateVars);
// Variables can be defined inline as well.
const { data } = await getCirclePricingState({ circleId: ..., });

// You can also pass in a `DataConnect` instance to the action shortcut function.
const dataConnect = getDataConnect(connectorConfig);
const { data } = await getCirclePricingState(dataConnect, getCirclePricingStateVars);

console.log(data.circle);
console.log(data.circleActivations);

// Or, you can use the `Promise` API.
getCirclePricingState(getCirclePricingStateVars).then((response) => {
  const data = response.data;
  console.log(data.circle);
  console.log(data.circleActivations);
});
...
### Using `GetCirclePricingState`'s `QueryRef` function

```typescript
import { getDataConnect, executeQuery } from 'firebase/data-connect';
import { connectorConfig, getCirclePricingStateRef, GetCirclePricingStateVariables } from '@bondcircle/dataconnect';
...
// Call the `getCirclePricingStateRef()` function to get a reference to the query.
const ref = getCirclePricingStateRef(getCirclePricingStateVars);
// Variables can be defined inline as well.
const ref = getCirclePricingStateRef({ circleId: ..., });

// You can also pass in a `DataConnect` instance to the `QueryRef` function.
const dataConnect = getDataConnect(connectorConfig);
const ref = getCirclePricingStateRef(dataConnect, getCirclePricingStateVars);
...
console.log(data.trialUsageTotals);
console.log(data.pricingDefinitions);
console.log(data.circleActivations);
console.log(data.pricingChangeAudits);
...
You can execute the `ClaimTrialAndPublishCircle` mutation using the following action shortcut function, or by calling `executeMutation()` after calling the following `MutationRef` function, both of which are defined in [dataconnect/index.d.ts](./index.d.ts):
```typescript
claimTrialAndPublishCircle(vars: ClaimTrialAndPublishCircleVariables): MutationPromise<ClaimTrialAndPublishCircleData, ClaimTrialAndPublishCircleVariables>;
...
You can also pass in a `DataConnect` instance to the action shortcut function or `MutationRef` function.
```typescript
claimTrialAndPublishCircle(dc: DataConnect, vars: ClaimTrialAndPublishCircleVariables): MutationPromise<ClaimTrialAndPublishCircleData, ClaimTrialAndPublishCircleVariables>;
...
If you need the name of the operation without creating a ref, you can retrieve the operation name by calling the `operationName` property on the claimTrialAndPublishCircleRef:
```typescript
const name = claimTrialAndPublishCircleRef.operationName;
console.log(name);
```
...
```
### Return Type
Recall that executing the `ClaimTrialAndPublishCircle` mutation returns a `MutationPromise` that resolves to an object with a `data` property.

The `data` property is an object of type `ClaimTrialAndPublishCircleData`, which is defined in [dataconnect/index.d.ts](./index.d.ts). It has the following fields:
```typescript
...
```
### Using `ClaimTrialAndPublishCircle`'s action shortcut function

```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, claimTrialAndPublishCircle, ClaimTrialAndPublishCircleVariables } from '@bondcircle/dataconnect';

// The `ClaimTrialAndPublishCircle` mutation requires an argument of type `ClaimTrialAndPublishCircleVariables`:
...
// Call the `claimTrialAndPublishCircle()` function to execute the mutation.
// You can use the `await` keyword to wait for the promise to resolve.
const { data } = await claimTrialAndPublishCircle(claimTrialAndPublishCircleVars);
// Variables can be defined inline as well.
const { data } = await claimTrialAndPublishCircle({ activationId: ..., creatorId: ..., circleId: ..., circleEntityId: ..., planDefinitionId: ..., circleType: ..., activatedAt: ..., });

// You can also pass in a `DataConnect` instance to the action shortcut function.
const dataConnect = getDataConnect(connectorConfig);
const { data } = await claimTrialAndPublishCircle(dataConnect, claimTrialAndPublishCircleVars);

console.log(data.creatorTrialUsage_insert);
console.log(data.circleActivation_insert);
console.log(data.circle_update);
console.log(data.circleAuditEntry_insert);
console.log(data.activityLog_insert);

// Or, you can use the `Promise` API.
claimTrialAndPublishCircle(claimTrialAndPublishCircleVars).then((response) => {
  const data = response.data;
  console.log(data.creatorTrialUsage_insert);
  console.log(data.circleActivation_insert);
  console.log(data.circle_update);
...
### Using `ClaimTrialAndPublishCircle`'s `MutationRef` function

```typescript
import { getDataConnect, executeMutation } from 'firebase/data-connect';
import { connectorConfig, claimTrialAndPublishCircleRef, ClaimTrialAndPublishCircleVariables } from '@bondcircle/dataconnect';
...
// Call the `claimTrialAndPublishCircleRef()` function to get a reference to the mutation.
const ref = claimTrialAndPublishCircleRef(claimTrialAndPublishCircleVars);
// Variables can be defined inline as well.
const ref = claimTrialAndPublishCircleRef({ activationId: ..., creatorId: ..., circleId: ..., circleEntityId: ..., planDefinitionId: ..., circleType: ..., activatedAt: ..., });

// You can also pass in a `DataConnect` instance to the `MutationRef` function.
const dataConnect = getDataConnect(connectorConfig);
const ref = claimTrialAndPublishCircleRef(dataConnect, claimTrialAndPublishCircleVars);
...
You can execute the `CreateCircleActivationAttempt` mutation using the following action shortcut function, or by calling `executeMutation()` after calling the following `MutationRef` function, both of which are defined in [dataconnect/index.d.ts](./index.d.ts):
```typescript
createCircleActivationAttempt(vars: CreateCircleActivationAttemptVariables): MutationPromise<CreateCircleActivationAttemptData, CreateCircleActivationAttemptVariables>;
...
You can also pass in a `DataConnect` instance to the action shortcut function or `MutationRef` function.
```typescript
createCircleActivationAttempt(dc: DataConnect, vars: CreateCircleActivationAttemptVariables): MutationPromise<CreateCircleActivationAttemptData, CreateCircleActivationAttemptVariables>;
...
If you need the name of the operation without creating a ref, you can retrieve the operation name by calling the `operationName` property on the createCircleActivationAttemptRef:
```typescript
const name = createCircleActivationAttemptRef.operationName;
console.log(name);
```
...
```
### Return Type
Recall that executing the `CreateCircleActivationAttempt` mutation returns a `MutationPromise` that resolves to an object with a `data` property.

The `data` property is an object of type `CreateCircleActivationAttemptData`, which is defined in [dataconnect/index.d.ts](./index.d.ts). It has the following fields:
```typescript
...
```
### Using `CreateCircleActivationAttempt`'s action shortcut function

```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, createCircleActivationAttempt, CreateCircleActivationAttemptVariables } from '@bondcircle/dataconnect';

// The `CreateCircleActivationAttempt` mutation requires an argument of type `CreateCircleActivationAttemptVariables`:
...
// Call the `createCircleActivationAttempt()` function to execute the mutation.
// You can use the `await` keyword to wait for the promise to resolve.
const { data } = await createCircleActivationAttempt(createCircleActivationAttemptVars);
// Variables can be defined inline as well.
const { data } = await createCircleActivationAttempt({ activationId: ..., circleId: ..., creatorId: ..., planDefinitionId: ..., activationType: ..., circleType: ..., tier: ..., listPriceMinor: ..., amountDueMinor: ..., provider: ..., providerReference: ..., previousActivationId: ..., createdAt: ..., });

// You can also pass in a `DataConnect` instance to the action shortcut function.
const dataConnect = getDataConnect(connectorConfig);
const { data } = await createCircleActivationAttempt(dataConnect, createCircleActivationAttemptVars);

console.log(data.circleActivation_insert);
console.log(data.circle_update);

// Or, you can use the `Promise` API.
createCircleActivationAttempt(createCircleActivationAttemptVars).then((response) => {
  const data = response.data;
  console.log(data.circleActivation_insert);
  console.log(data.circle_update);
});
...
### Using `CreateCircleActivationAttempt`'s `MutationRef` function

```typescript
import { getDataConnect, executeMutation } from 'firebase/data-connect';
import { connectorConfig, createCircleActivationAttemptRef, CreateCircleActivationAttemptVariables } from '@bondcircle/dataconnect';
...
// Call the `createCircleActivationAttemptRef()` function to get a reference to the mutation.
const ref = createCircleActivationAttemptRef(createCircleActivationAttemptVars);
// Variables can be defined inline as well.
const ref = createCircleActivationAttemptRef({ activationId: ..., circleId: ..., creatorId: ..., planDefinitionId: ..., activationType: ..., circleType: ..., tier: ..., listPriceMinor: ..., amountDueMinor: ..., provider: ..., providerReference: ..., previousActivationId: ..., createdAt: ..., });

// You can also pass in a `DataConnect` instance to the `MutationRef` function.
const dataConnect = getDataConnect(connectorConfig);
const ref = createCircleActivationAttemptRef(dataConnect, createCircleActivationAttemptVars);
...
You can execute the `FailCircleActivationAttempt` mutation using the following action shortcut function, or by calling `executeMutation()` after calling the following `MutationRef` function, both of which are defined in [dataconnect/index.d.ts](./index.d.ts):
```typescript
failCircleActivationAttempt(vars: FailCircleActivationAttemptVariables): MutationPromise<FailCircleActivationAttemptData, FailCircleActivationAttemptVariables>;
...
You can also pass in a `DataConnect` instance to the action shortcut function or `MutationRef` function.
```typescript
failCircleActivationAttempt(dc: DataConnect, vars: FailCircleActivationAttemptVariables): MutationPromise<FailCircleActivationAttemptData, FailCircleActivationAttemptVariables>;
...
If you need the name of the operation without creating a ref, you can retrieve the operation name by calling the `operationName` property on the failCircleActivationAttemptRef:
```typescript
const name = failCircleActivationAttemptRef.operationName;
console.log(name);
```
...
```
### Return Type
Recall that executing the `FailCircleActivationAttempt` mutation returns a `MutationPromise` that resolves to an object with a `data` property.

The `data` property is an object of type `FailCircleActivationAttemptData`, which is defined in [dataconnect/index.d.ts](./index.d.ts). It has the following fields:
```typescript
...
```
### Using `FailCircleActivationAttempt`'s action shortcut function

```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, failCircleActivationAttempt, FailCircleActivationAttemptVariables } from '@bondcircle/dataconnect';

// The `FailCircleActivationAttempt` mutation requires an argument of type `FailCircleActivationAttemptVariables`:
...
// Call the `failCircleActivationAttempt()` function to execute the mutation.
// You can use the `await` keyword to wait for the promise to resolve.
const { data } = await failCircleActivationAttempt(failCircleActivationAttemptVars);
// Variables can be defined inline as well.
const { data } = await failCircleActivationAttempt({ activationId: ..., circleId: ..., failureCode: ..., failedAt: ..., });

// You can also pass in a `DataConnect` instance to the action shortcut function.
const dataConnect = getDataConnect(connectorConfig);
const { data } = await failCircleActivationAttempt(dataConnect, failCircleActivationAttemptVars);

console.log(data.circleActivation_update);
console.log(data.circle_update);

// Or, you can use the `Promise` API.
failCircleActivationAttempt(failCircleActivationAttemptVars).then((response) => {
  const data = response.data;
  console.log(data.circleActivation_update);
  console.log(data.circle_update);
});
...
### Using `FailCircleActivationAttempt`'s `MutationRef` function

```typescript
import { getDataConnect, executeMutation } from 'firebase/data-connect';
import { connectorConfig, failCircleActivationAttemptRef, FailCircleActivationAttemptVariables } from '@bondcircle/dataconnect';
...
// Call the `failCircleActivationAttemptRef()` function to get a reference to the mutation.
const ref = failCircleActivationAttemptRef(failCircleActivationAttemptVars);
// Variables can be defined inline as well.
const ref = failCircleActivationAttemptRef({ activationId: ..., circleId: ..., failureCode: ..., failedAt: ..., });

// You can also pass in a `DataConnect` instance to the `MutationRef` function.
const dataConnect = getDataConnect(connectorConfig);
const ref = failCircleActivationAttemptRef(dataConnect, failCircleActivationAttemptVars);
...
You can execute the `CompletePaidCircleActivation` mutation using the following action shortcut function, or by calling `executeMutation()` after calling the following `MutationRef` function, both of which are defined in [dataconnect/index.d.ts](./index.d.ts):
```typescript
completePaidCircleActivation(vars: CompletePaidCircleActivationVariables): MutationPromise<CompletePaidCircleActivationData, CompletePaidCircleActivationVariables>;
...
You can also pass in a `DataConnect` instance to the action shortcut function or `MutationRef` function.
```typescript
completePaidCircleActivation(dc: DataConnect, vars: CompletePaidCircleActivationVariables): MutationPromise<CompletePaidCircleActivationData, CompletePaidCircleActivationVariables>;
...
If you need the name of the operation without creating a ref, you can retrieve the operation name by calling the `operationName` property on the completePaidCircleActivationRef:
```typescript
const name = completePaidCircleActivationRef.operationName;
console.log(name);
```
...
```
### Return Type
Recall that executing the `CompletePaidCircleActivation` mutation returns a `MutationPromise` that resolves to an object with a `data` property.

The `data` property is an object of type `CompletePaidCircleActivationData`, which is defined in [dataconnect/index.d.ts](./index.d.ts). It has the following fields:
```typescript
...
```
### Using `CompletePaidCircleActivation`'s action shortcut function

```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, completePaidCircleActivation, CompletePaidCircleActivationVariables } from '@bondcircle/dataconnect';

// The `CompletePaidCircleActivation` mutation requires an argument of type `CompletePaidCircleActivationVariables`:
...
// Call the `completePaidCircleActivation()` function to execute the mutation.
// You can use the `await` keyword to wait for the promise to resolve.
const { data } = await completePaidCircleActivation(completePaidCircleActivationVars);
// Variables can be defined inline as well.
const { data } = await completePaidCircleActivation({ activationId: ..., circleId: ..., creatorId: ..., pricePaidMinor: ..., activatedAt: ..., });

// You can also pass in a `DataConnect` instance to the action shortcut function.
const dataConnect = getDataConnect(connectorConfig);
const { data } = await completePaidCircleActivation(dataConnect, completePaidCircleActivationVars);
...
// Or, you can use the `Promise` API.
completePaidCircleActivation(completePaidCircleActivationVars).then((response) => {
  const data = response.data;
  console.log(data.circleActivation_update);
  console.log(data.circle_update);
  console.log(data.circleAuditEntry_insert);
...
### Using `CompletePaidCircleActivation`'s `MutationRef` function

```typescript
import { getDataConnect, executeMutation } from 'firebase/data-connect';
import { connectorConfig, completePaidCircleActivationRef, CompletePaidCircleActivationVariables } from '@bondcircle/dataconnect';
...
// Call the `completePaidCircleActivationRef()` function to get a reference to the mutation.
const ref = completePaidCircleActivationRef(completePaidCircleActivationVars);
// Variables can be defined inline as well.
const ref = completePaidCircleActivationRef({ activationId: ..., circleId: ..., creatorId: ..., pricePaidMinor: ..., activatedAt: ..., });

// You can also pass in a `DataConnect` instance to the `MutationRef` function.
const dataConnect = getDataConnect(connectorConfig);
const ref = completePaidCircleActivationRef(dataConnect, completePaidCircleActivationVars);
...
You can execute the `CompleteCirclePlanUpgrade` mutation using the following action shortcut function, or by calling `executeMutation()` after calling the following `MutationRef` function, both of which are defined in [dataconnect/index.d.ts](./index.d.ts):
```typescript
completeCirclePlanUpgrade(vars: CompleteCirclePlanUpgradeVariables): MutationPromise<CompleteCirclePlanUpgradeData, CompleteCirclePlanUpgradeVariables>;
...
You can also pass in a `DataConnect` instance to the action shortcut function or `MutationRef` function.
```typescript
completeCirclePlanUpgrade(dc: DataConnect, vars: CompleteCirclePlanUpgradeVariables): MutationPromise<CompleteCirclePlanUpgradeData, CompleteCirclePlanUpgradeVariables>;
...
If you need the name of the operation without creating a ref, you can retrieve the operation name by calling the `operationName` property on the completeCirclePlanUpgradeRef:
```typescript
const name = completeCirclePlanUpgradeRef.operationName;
console.log(name);
```
...
```
### Return Type
Recall that executing the `CompleteCirclePlanUpgrade` mutation returns a `MutationPromise` that resolves to an object with a `data` property.

The `data` property is an object of type `CompleteCirclePlanUpgradeData`, which is defined in [dataconnect/index.d.ts](./index.d.ts). It has the following fields:
```typescript
...
```
### Using `CompleteCirclePlanUpgrade`'s action shortcut function

```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, completeCirclePlanUpgrade, CompleteCirclePlanUpgradeVariables } from '@bondcircle/dataconnect';

// The `CompleteCirclePlanUpgrade` mutation requires an argument of type `CompleteCirclePlanUpgradeVariables`:
...
// Call the `completeCirclePlanUpgrade()` function to execute the mutation.
// You can use the `await` keyword to wait for the promise to resolve.
const { data } = await completeCirclePlanUpgrade(completeCirclePlanUpgradeVars);
// Variables can be defined inline as well.
const { data } = await completeCirclePlanUpgrade({ activationId: ..., circleId: ..., creatorId: ..., targetPlan: ..., targetPlanDefinitionId: ..., targetMemberLimit: ..., targetActivationPrice: ..., targetActivationPriceMinor: ..., pricePaidMinor: ..., activatedAt: ..., });

// You can also pass in a `DataConnect` instance to the action shortcut function.
const dataConnect = getDataConnect(connectorConfig);
const { data } = await completeCirclePlanUpgrade(dataConnect, completeCirclePlanUpgradeVars);
...
// Or, you can use the `Promise` API.
completeCirclePlanUpgrade(completeCirclePlanUpgradeVars).then((response) => {
  const data = response.data;
  console.log(data.circleActivation_update);
  console.log(data.circle_update);
  console.log(data.circleAuditEntry_insert);
...
### Using `CompleteCirclePlanUpgrade`'s `MutationRef` function

```typescript
import { getDataConnect, executeMutation } from 'firebase/data-connect';
import { connectorConfig, completeCirclePlanUpgradeRef, CompleteCirclePlanUpgradeVariables } from '@bondcircle/dataconnect';
...
// Call the `completeCirclePlanUpgradeRef()` function to get a reference to the mutation.
const ref = completeCirclePlanUpgradeRef(completeCirclePlanUpgradeVars);
// Variables can be defined inline as well.
const ref = completeCirclePlanUpgradeRef({ activationId: ..., circleId: ..., creatorId: ..., targetPlan: ..., targetPlanDefinitionId: ..., targetMemberLimit: ..., targetActivationPrice: ..., targetActivationPriceMinor: ..., pricePaidMinor: ..., activatedAt: ..., });

// You can also pass in a `DataConnect` instance to the `MutationRef` function.
const dataConnect = getDataConnect(connectorConfig);
const ref = completeCirclePlanUpgradeRef(dataConnect, completeCirclePlanUpgradeVars);
